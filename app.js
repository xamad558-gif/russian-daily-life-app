...
function populateSubcategories(){
  const seen = new Set();
  state.words.forEach(w => seen.add(w.subCategory));
  for (const sub of seen){
    const opt = document.createElement("option");
    opt.value = sub;
    els.subCategoryFilter.appendChild(opt);
  }
}
function applyUiLanguage(lang){
  ...
  els.sortFilter.options[0].text = t.sortPopular;
  els.sortFilter.options[1].text = t.sortAZ;
...
function renderCategoryMenu(){
  ...
  const counts = {};
  state.words.forEach(w => counts[w.subCategory] = (counts[w.subCategory] || 0) + 1);
  els.categoryMenu.innerHTML = Object.keys(counts).map((sub, idx) => `
    <button class="category-item ${idx===0 ? "active":""}" type="button" data-submenu="${sub}">
...
function switchView(view){
  ...
  $$("[data-view-btn]").forEach(btn => btn.classList.toggle("active", btn.dataset.viewBtn === view));
  ["vocabulary","wordDetail","review","quiz","progress"].forEach(v => {
    const panel = $(`#${v}View`);
...
function applyFilters(){
  const q = normalize(els.searchInput.value), sub = els.subCategoryFilter.value, level = els.levelFilter.value, sort = els.sortFilter.value;
  state.filtered = state.words.filter(w => {
    const hay = normalize([w.russian,w.transliteration,w.arabic,w.english,w.exampleRu,w.exampleAr,w.exampleEn].join(" "));
    return (!q || hay.includes(q)) && (sub === "all" || w.subCategory === sub) && (level === "all" || w.level === level);
  });
  if (sort === "az") state.filtered.sort((a,b)=>a.russian.localeCompare(b.russian,"ru"));
  if (sort === "za") state.filtered.sort((a,b)=>b.russian.localeCompare(a.russian,"ru"));
  if (sort === "default") state.filtered.sort((a,b)=>(b.frequency||0)-(a.frequency||0));
  if (sort === "mastery") state.filtered.sort((a,b)=>(state.mastery[a.id]||0)-(state.mastery[b.id]||0));
  if (!state.filtered.find(w => w.id === state.selectedWordId)) state.selectedWordId = state.filtered[0]?.id || null;
  const t = I18N[state.uiLang];
  els.pageCounter.textContent = `${state.filtered.length} ${t.pageCounterWord}`;
...
function bindCardButtons(root){
  ...
  root.querySelectorAll("[data-audio]").forEach(btn => btn.addEventListener("click", e => { e.stopPropagation(); playAudio(btn.dataset.id, btn.dataset.audio, btn.dataset.lang); }));
}
  ...
  els.detailPane.innerHTML = `
    ...
    <div class="detail-block">
      ...
      <div class="detail-info-grid">
        ...
        <span class="label">${escapeHTML(t.labels.updated)}</span><span>${escapeHTML(w.addedAt || "2026-07-05")}</span>
      </div>
      <div class="mastery-actions">
        <button class="mastery-btn known" data-known="${w.id}" type="button">${escapeHTML(t.labels.known)}</button>
        <button class="mastery-btn review" data-review="${w.id}" type="button">${escapeHTML(t.labels.review)}</button>
      </div>
    </div>`;
  els.detailPane.querySelectorAll("[data-audio]").forEach(btn => btn.addEventListener("click", () => playAudio(btn.dataset.id, btn.dataset.audio, btn.dataset.lang)));
  const openBtn = els.detailPane.querySelector("[data-open-detail]");
  if (openBtn) openBtn.addEventListener("click", () => openFullDetail(openBtn.dataset.openDetail));
  els.detailPane.querySelector("[data-known]").addEventListener("click", () => changeMastery(w.id, +25));
  els.detailPane.querySelector("[data-review]").addEventListener("click", () => { toggleFavorite(w.id, true); changeMastery(w.id, -10); });
}
...
function renderFullWordDetail(){
  const w = state.words.find(x => x.id === state.selectedWordId);
  if (!w || !els.fullWordDetail) return;
...
function playExampleAudio(w, index, lang){
  const ex = (w.examples || [])[index];
  if (!ex) return;
  const text = ex[lang] || "";
  const src = ex.audio && ex.audio[lang];
...
function renderReview(){
  const savedWords = state.words.filter(w => state.saved.has(w.id));
...
function updateMetrics(){
  ...
  els.reviewCount.textContent = state.saved.size;
  const vals = state.words.map(w => state.mastery[w.id] || 0);
  const avg = vals.length ? Math.round(vals.reduce((a,b)=>a+b,0)/vals.length) : 0;
  const mastered = vals.filter(v => v >= 80).length;
